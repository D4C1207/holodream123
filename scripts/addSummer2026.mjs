/**
 * Add シンクロする夏のスパークル ★5 cards (Miko & Suisei swimsuits).
 * Source: Gamerch (2026/08/07)
 */
import fs from "fs";
...
function parseSpecial(text) {
  ...
  const condRate = t.match(
    /(ライフ\d+以上|\d+コンボ以上|(0期生|1期生|2期生|3期生|4期生|5期生|ゲーマーズ|holoX|ID1期生|ID2期生|ID3期生|Myth|Promise|Advent|ReGLOSS)が\d+人以上)でスキル発動率が(\d+)%/,
  );
  const rate = t.match(/スキル発動率が(\d+)%/);
  return {
    duration: durationMatch ? +durationMatch[1] : 0,
    scoreSupport: support ? +support[1] : 0,
    skillRate: condRate ? +condRate[3] : rate ? +rate[1] : 0,
...
const cards = [
  {
    member: "さくらみこ",
    costumeName: "ビーチで弾ける、光彩ショット！",
    rarity: 5,
    typeJp: "ピュア",
    unit: "0期生",
    stats: { performance: 11145, technique: 7828, sense: 6947, total: 25920 },
    special: "11秒間スコアサポート効果145%",
    active: "27秒毎に中確率で10秒間スコアが115%UP",
    passive: "ピュアタイプ2人以上でピュアタイプ2人のパフォーマンスが43%UP",
    costumeSkill:
      "ピュアタイプ2人以上で全員のパフォーマンスが80%UP、ピュアタイプ2人以上で全員のスコアサポート効果25%UP",
  },
  {
    member: "星街すいせい",
    costumeName: "夏に一閃！水鉄砲のアルペジオ",
    rarity: 5,
    typeJp: "キュート",
    unit: "0期生",
    stats: { performance: 7828, technique: 6947, sense: 11145, total: 25920 },
    special: "12秒間スコアサポート効果120% 0期生が2人以上でスキル発動率が45%UP",
    active: "23秒毎に中確率で8秒間スコアが120%UP",
    passive: "0期生が2人以上で0期生2人のスコアサポート効果12%",
    costumeSkill: "0期生が2人以上で全員のセンスが135%UP",
  },
];
...
console.log({
  ...
  miko: data.cards.find((c) => c.costumeName === "ビーチで弾ける、光彩ショット！"),
  suisei: data.cards.find((c) => c.costumeName === "夏に一閃！水鉄砲のアルペジオ"),
});

...
const zh: Messages = {
  ...
  rosterCardPickNote: "有多張卡面的角色請在這裡勾選實際持有的★5／活動卡（可多選）；勾哪張就代表同時擁有該張卡對應衣裝。",
...
const en: Messages = {
  ...
  rosterCardPickNote: "For members with multiple cards, select every ★5/event card you actually own. Each selected card also unlocks its matching costume.",
...
const ja: Messages = {
  ...
  rosterCardPickNote: "複数カードがあるメンバーは、実際に所持する★5／イベントカードをここで選択してください。選んだカードに対応する衣装だけが使用可能になります。",
